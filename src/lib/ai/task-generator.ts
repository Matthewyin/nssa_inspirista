...
export class AITaskGenerator {
  ...
  async generateTaskPlan(prompt: string, aiConfig?: AiConfig, apiKey?: string, timeframe?: number): Promise<TaskPlan> {
    ...
    const systemPrompt = `你是一位经验丰富的项目管理专家和学习规划师，擅长将复杂目标分解为可执行的具体行动计划。
...
## 分析框架
1. **目标解构**：将大目标拆解为核心技能点和知识模块
2. **时间规划**：基于学习曲线合理分配时间，避免过于紧张或松散
3. **难度递进**：从基础概念到实践应用，循序渐进
4. **检验标准**：每个里程碑都有明确的完成标准和验收方式

## 输出规范
- 总体规划：简洁有力，突出核心价值和最终成果（15字内）
- 里程碑设计：每个里程碑包含具体任务、学习方法、实践项目和验收标准
- 时间分配：考虑学习难度、个人基础、实际可用时间
- 标签选择：反映技能领域、学习方式、应用场景
...
里程碑计划：
里程碑1（第1-X天）：[具体学习内容] + [实践任务] + [验收标准]
里程碑2（第X-Y天）：[具体学习内容] + [实践任务] + [验收标准]
里程碑3（第Y-Z天）：[具体学习内容] + [实践任务] + [验收标准]

推荐标签：#技能领域 #学习方式`;

    const userPrompt = `## 用户目标分析
**目标描述**：${prompt}
**时间范围**：${extractedTimeframe}天
**期望输出**：制定一个科学合理、可执行性强的学习/工作计划

## 分析要求
请深入分析这个目标的：
1. **复杂度评估**：需要掌握哪些核心技能和知识点？
2. **时间合理性**：${extractedTimeframe}天的时间是否充足？如何优化时间分配？
3. **学习路径**：最佳的学习顺序和方法是什么？
4. **实践应用**：如何通过实际项目巩固学习成果？
5. **验收标准**：每个阶段如何验证学习效果？

请基于以上分析，生成一个高质量的任务计划。`;
